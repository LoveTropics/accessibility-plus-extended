...
public class POIGui extends LightweightGuiDescription {
  ...
  public POIGui(LocalPlayer player, Minecraft client) {
    this.player = player;
    this.client = client;
    ...
    WButton back = new WButton(new TranslatableComponent("gui.apextended.config.buttons.back"));
    back.setOnClick(this::onBackClick);
    root.add(back, 2, 15, 7, 1);

    WButton done = new WButton(new TranslatableComponent("gui.apextended.config.buttons.done"));
